...
function OrdersView({ orders, onOpen, onAction }){
  ...
  const filtered = orders.filter((o) => {
    if (date && ymd(o.createdAt) !== date) return false;
    if (query){
    ...
    <div className="ordersview">
      <div className="ordertools">
        ...
        <div className="ot-date">
          ...
          <span className="dlab">التاريخ</span>
          <input type="date" value={date} max={today} onChange={(e) => setDate(e.target.value)} />
        </div>
        {date && (
          <span className="ot-chip">
            {date === today ? "اليوم" : prettyDate(date)}
            <button onClick={() => setDate("")} title="إلغاء"><Ic.x s={13} /></button>
          </span>
        )}
        <span className="ot-count tnum">{shownCount} {shownCount === 1 ? "طلب" : "طلب"}{active ? " · نتيجة الفلترة" : ""}</span>
      </div>
...
function OrderDrawer({ order, onClose, onAction }){
  ...
  const cur = rejected ? 1 : order_rank[order.status];
      ...
      <div className="drawer">
        ...
        <div className="drawer-body">
          ...
          <table className="dtable">
            <tbody>
              {order.items.map((it, i) => (
                <tr key={i}>
                  <td className="q tnum">×{it.qty}</td>
                  <td>
                    {it.name}
                    {it.mods && it.mods.length > 0 && <div className="mod">{it.mods.join("، ")}</div>}
                  </td>
                  <td className="pr tnum">{S.money(it.price * it.qty)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="timeline">
            {steps.map((s2, i) => {
              ...
              return (
                <div className={"tl " + (done ? "done" : isCur ? "cur" : "") + (s2.k === "rejected" ? " tl-rej" : "")} key={s2.k}>
                  <div className="tl-mk">
                  ...
                  <div className="tl-c">
                    ...
                    <div className="tl-t tnum">{hasTime ? fmtTime(s2.time) : (isCur ? "الآن" : "بانتظار")}</div>
        ...
        <div className="drawer-foot">
          ...
          {order.status === "preparing" && (
            <>
              <button className="btn btn-line" onClick={() => onAction(order, "ready")} style={{ flex: 1 }}>
                <Ic.check s={19} /> جاهز
              </button>
            </>
          )}
      ...
      <div className="modal" style={{ width: "min(440px,94vw)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-foot">
          ...
          <button className="btn btn-gold btn-accept" disabled={!valid} style={{ opacity: valid ? 1 : 0.5 }}
            onClick={() => valid && onSave({ name: name.trim(), price: Number(price), cat, available })}>
            <Ic.check s={19} /> حفظ
          </button>
        </div>
      </div>
