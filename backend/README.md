...
## البريد الإلكتروني (التأكيد) · Email

لا يوجد دفع إلكتروني حالياً — الطلب **يُؤكَّد فوراً** ويُرسَل تأكيد إلى بريد الزبون،
وتُرسَل تحديثات الحالة (في الطريق / تم التسليم / رفض / إلغاء) أيضاً.

- بدون إعداد: الرسائل تُكتب في **صندوق صادر محلّي** (`backend/outbox/` + سجلّ) ويمكن
  للإدارة عرضها عبر `GET /api/emails` — مناسب للتجربة بلا خادم بريد.
- للإنتاج: اضبط متغيّرات البيئة وسيُرسل عبر **SMTP** حقيقي (عبر `nodemailer` —
  `npm i nodemailer`):

```bash
SMTP_HOST=smtp.example.com SMTP_PORT=587 SMTP_USER=... SMTP_PASS=... \
MAIL_FROM="Sonbol <no-reply@yourdomain>" npm start
```

`backend/email.js` يحوي النقل والقوالب. الزبون يسجّل ببريد إلكتروني (مطلوب).
